...
elseif ($_REQUEST['act'] == 'quick')
{
    /* 检查权限 */
    admin_priv('order_view');

    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['021_order_list']);
    $smarty->assign('action_link', '');

    $smarty->assign('full_page',        1);

    $order_list = order_quick_list();
    $smarty->assign('order_list',   $order_list['orders']);
    $smarty->assign('filter',       $order_list['filter']);
    $smarty->assign('record_count', $order_list['record_count']);
    $smarty->assign('page_count',   $order_list['page_count']);
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_quick_list.htm');
}
/*------------------------------------------------------ */
//-- 排序、分页、查询
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'quick_view'){
    $quick_id = isset($_GET['quick_id']) ? intval($_GET['quick_id']) : 0;
    if (!$quick_id){
        sys_msg('ID参数有误');
        exit;
    }
    
    $data = $db->getRow("select * from {$ecs->table('order_quick')} where quick_id='{$quick_id}'");
    ...
    $sql = "SELECT MAX(quick_id) FROM " . $ecs->table('order_quick') . " as o WHERE quick_id < '$quick_id'";
    //     if ($agency_id > 0)
        //     {
    ...
    $sql = "SELECT MIN(quick_id) FROM " . $ecs->table('order_quick') . " as o WHERE quick_id > '$quick_id'";
    //     if ($agency_id > 0)
        //     {
        //         $sql .= " AND agency_id = '$agency_id'";
        //     }
    ...
    $smarty->assign('user', $db->getRow("select * from {$ecs->table('users')} where user_id='{$data['user_id']}'"));
    
    $_REQUEST['order_id'] = $data['order_id'];
    
    if (intval($_REQUEST['order_id'])){
    /* 根据订单id或订单号查询订单信息 */
...
//     }
    ...
    elseif (isset($_GET['shipping_print']))
    {
        ...
        if ($shipping['print_model'] == 2)
        {
            ...
            $smarty->assign('lable_box', $lable_box);
    ...
    $smarty->assign('action_link', array('href' => 'order.php?act=quick', 'text' => $_LANG['02_order_list']));
    
    $smarty->display('order_quick_info.htm');
    
}

elseif ($_REQUEST['act'] == 'quick_delete'){
    ...
    $db->query("delete from {$ecs->table('order_quick')} where quick_id='{$quick_id}'");
    if ($db->affected_rows()){
        if (!empty($data['images'])){
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('other' == $step)
    {
        ...
        $inv_payee     = str_replace(PHP_EOL, "#", $_POST['inv_payee']);
        $arr = explode("#", $inv_payee);
        $str = "";
        foreach ($arr as $val) {
            $val = trim($val);
            if($val){
        ...
        $order['inv_payee']      = $str;
        // var_dump(strpos($order['inv_payee'], PHP_EOL));
        // var_dump($order['inv_payee']);exit;
    ...
    elseif ('money' == $step)
    {
        ...
        $order['cur_order_amount']      = isset($_POST['order_amount']) && floatval($_POST['order_amount']) >= 0 ? round(floatval($_POST['order_amount']), 2) : 0;
        ...
        if($order['cur_order_amount'] != $order['order_amount']){
            $order['order_amount'] = $order['cur_order_amount'];
        }
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        $order = order_info($order_id);
        $order['inv_payee'] = str_replace("<br>", PHP_EOL.PHP_EOL, $order['inv_payee']);
        /* 发货单格式化 */
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['suppliers'] = empty($_REQUEST['suppliers']) ? '' : trim($_REQUEST['suppliers']);
        ...
        if ($filter['suppliers'])
        {
            $sql = "select suppliers_id from " . $GLOBALS['ecs']->table('suppliers') . " where suppliers_name LIKE '%{$filter['suppliers']}%'";
            $suppliers = $GLOBALS['db']->getAll($sql);
            $id_str = "";
            if($suppliers){
                foreach ($suppliers as $val) {
                   $id_str .= "'".$val['suppliers_id']."',";
                }
                $id_str = rtrim($id_str, ',');
                $where .= " AND o.supplier_id in({$id_str})";
            }
        }
    ...
    {
        $row[$key]['no_formated_order_amount'] = $value['order_amount'];
...
function order_quick_list(){
    $result = get_filter();
    if ($result === false)
    {
        /* 过滤信息 */
        $filter['realname'] = empty($_REQUEST['realname']) ? '' : trim($_REQUEST['realname']);
        ...
        if ($filter['realname'])
        {
            $where .= " AND realname LIKE '%" . mysql_like_quote($filter['realname']) . "%'";
        }
        if ($filter['mobile'])
        {
            $where .= " AND mobile LIKE '%" . mysql_like_quote($filter['mobile']) . "%'";
        }
        
        if ($filter['start_time'])
        {
            $where .= " AND ctime >= '$filter[start_time]'";
        }
        if ($filter['end_time'])
        {
            $where .= " AND ctime <= '$filter[end_time]'";
        }
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_quick') . $where;
        
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('order_quick') . $where .
                " order by quick_id desc LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
